...
bool cstream_cell_init(struct cstream_cell_t *cell) {
  CHECK_CELL_PTR("cstream_cell_init", false);

  char *value = calloc(10, sizeof(char));

  if (!value) {
    LOG_WARNING("cstream_cell_init",
                "value could not be allocated and is NULL.");
    return false;
  }

  *cell = (struct cstream_cell_t){.type = CSTREAM_TYPE_NOTYPE, .value = value};

  return true;
}

bool cstream_cell_isnull(const struct cstream_cell_t *cell) {
  CHECK_CELL_PTR("cstream_cell_isnull", true);
  return cell->type == CSTREAM_TYPE_NULL;
}

int64_t cstream_cell_as_int64(const struct cstream_cell_t *cell) {
...
double cstream_cell_as_decimal(const struct cstream_cell_t *cell) {
...
const char *cstream_cell_as_string(const struct cstream_cell_t *cell) {
...
void cstream_cell_set_int64(struct cstream_cell_t *cell, const int64_t value,
  ...
  if (!value_str) {
    LOG_WARNING("cstream_cell_set_int64",
                "couldn't allocate memory for value.");
    return;
  }
...
void cstream_cell_set_decimal(struct cstream_cell_t *cell, const double value,
  ...
  char *value_str = calloc(value_len, sizeof(char));
  if (!value_str) {
    LOG_WARNING("cstream_cell_set_decimal",
                "couldn't allocate memory for value.");
    return;
  }
...
void cstream_cell_set_string(struct cstream_cell_t *cell, const char *value,
                             const bool write_to_file) {
  ...
  if (!value_str) {
    LOG_WARNING("cstream_cell_set_string",
                "couldn't allocate memory for value.");
    return;
  }
...
void cstream_cell_set_null(struct cstream_cell_t *cell,
                           const bool write_to_file) {
